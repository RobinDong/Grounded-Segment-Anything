...
def save_mask_data(output_dir, stem, image, mask_list, box_list, label_list):
    ...
    plt.axis('off')
    plt.savefig(os.path.join(output_dir, f"{stem}_mask.jpg"), bbox_inches="tight", dpi=300, pad_inches=0.0)'''

    mask_2d = mask_list.squeeze(0).squeeze(1) # shape: [height, width]
    mask_np = mask_2d.cpu().numpy()

    masked_image = image * mask_np[..., None]
    cv2.imwrite(os.path.join(output_dir, f"{stem}.jpg"), masked_image.squeeze())
...
if __name__ == "__main__":
    ...
    for directory in tqdm.tqdm(glob.glob("/data/imagenet/train/jpeg/*")):
